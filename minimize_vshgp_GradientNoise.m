 function [X, fX, i] = minimize_vshgp_GradientNoise(X, f, length, varargin)

% Minimize a differentiable multivariate function using conjugate gradients.
% Changkai Zhou added the gradient noise in line 95.
%
...
%%%%
%[u,v] = find(df0 == max(df0));
%fprintf('%s %4i; ', 'Linesearch', i);
%fprintf('%s %6i;  Value %4.6e, maxdf(id %5i) %4.6e, mindf %4.6e, meandf %4.6e\r', S, i, f0, u(1), max(df0), min(df0), mean(df0));
%%%%
...
while i < abs(length)                                      % while not finished
  ...
  df0 = df0 + normrnd(0,100/(1+i)^0.55,size(df0,1),1);       % add gredient noise
  ...
  if abs(d3) < -SIG*d0 && f3 < f0+x3*RHO*d0          % if line search succeeded
    ...
    %%%%
	%[u,v] = find(df0 == max(df0));
    %fprintf('%s %4i; ', 'Linesearch', count); count = count + 1;
    %fprintf('%s %6i;  Value %4.6e, maxdf(id %5i) %4.6e, mindf %4.6e, meandf %4.6e\r', S, i, f0, u(1), max(df0), min(df0), mean(df0));
    %%%%
    if exist('fflush','builtin') fflush(stdout); end
