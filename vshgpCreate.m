...
[n D] = size(X);
if n<options.m(1)
    options.m(1)=min(n,options.m(1));
end
if n<options.m(2)
    options.m(2)=min(n,options.m(2));
end
...
if strcmp(model.Pseudo.type, 'random')        
    ...
    if model.m(1) < model.n
        for i=1:D
            model.Pseudo.Xm(:,i) = linspace(min(X(:,i)),max(X(:,i)),model.m(1))';
        end
        perm = randperm(n);
        model.Pseudo.Xm = X(perm(1:model.m(1)),:);   
    ...
    if model.m(2) < model.n 
        for i=1:D
            model.Pseudo.Xu(:,i) = linspace(min(X(:,i)),max(X(:,i)),model.m(2))';
        end
        perm = randperm(n);
        model.Pseudo.Xu = X(perm(1:model.m(2)),:);
